...
form.addEventListener('submit', (e) => {
    ...
    keys.innerHTML += `<option>${ form.key.value }</option>` // aca agrego la opcion que completé en el form

    form.reset() //aca vacio los campos del formulario
})

keys.addEventListener('change', () => {
    document.getElementById('infoValue').textContent = //la primera parte nos muestra dentro de una etiqueta p con id infoValue el valor que seleccionamos mas abajo.
        localStorage.getItem(keys[ keys.selectedIndex ].textContent) // asi obtenemos el valor delo value
})

// localStorage.clear() // borra lo que hay en sessionStorage o en localStorage
